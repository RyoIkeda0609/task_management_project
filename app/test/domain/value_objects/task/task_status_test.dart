...
void main() {
  group('TaskStatus', () {
    ...
    group('fromString', () {
      test('有効な文字列からTaskStatusを生成できること', () {
        expect(TaskStatus.fromString('todo'), TaskStatus.todo);
        expect(TaskStatus.fromString('doing'), TaskStatus.doing);
        expect(TaskStatus.fromString('done'), TaskStatus.done);
      });

      test('不正な文字列でArgumentErrorが発生すること', () {
        expect(
          () => TaskStatus.fromString('invalid_value'),
          throwsArgumentError,
        );
        expect(() => TaskStatus.fromString('unknown'), throwsArgumentError);
