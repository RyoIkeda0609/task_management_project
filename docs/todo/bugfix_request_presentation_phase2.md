...
ゴール、マイルストーン、タスク編集画面で、編集をしようとした際に「更新」、「キャンセル」ボタンが非活性でローディングになり、更新できない
編集を2回連続で行おうとしたときにおこる(1回目編集する→成功を確認する→もう一回編集する→ボタンぐるぐる)
...
- タスク詳細から遷移した場合、1文字入力するごとにキーボードが強制的に閉じられてしまう
...
- 今日のタスクで表示するタスクのステータスと、マイルストーン詳細で表示するタスクのステータスが合わないときがある(マイルストーン詳細で完了にしても、本日のタスクに表示されない)
- マイルストーン詳細からタスクのステータスを更新しても、ホームの進捗欄に表示されない(変更が反映されない)
...
タスクやマイルストーンのステータスを更新してもホームの進捗が変更されない
