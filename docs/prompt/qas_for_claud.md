...
- 私は現在、Flutterを用いてアプリケーション開発をしています。
- 作成しているアプリは、「ゴールから逆算して日々のタスク達成を助けるタスク管理アプリ」です。
- ソースコードは /app/下に配置しています。
- ドキュメントは /docs/spec/下に配置しています。

### 依頼事項

- あなたはスーパーエンジニアとして、このアプリの実装を担当します。
- まずは現時点での実装を確認してください。ドキュメントとコードを確認して、以下の点を明らかにしてください。
  - 必要な機能が何パーセント実装されているか
  - クリーンなコードであるか
  - クリーンなアーキテクチャであるか
  - テストが意味のあるものになっているか（TDD的に、まず機能がきちんと定義されていて、それを満たすことを確認するテストが書かれ、それを満たすコードが書かれているか、のようなテストになっているか、実装を後追いしたようなあまり意味のないテストになっていないか）
  - かならず問題点はたくさん出るでしょうが、冷静に一つずつかいけつしていきましょう！
- そのうえで、問題点を少しずつクリアにしていきます。
  - 一度に全部修正するのは難しいので、無理にその場で修正しようとせず、少しずつ行きましょう！
  - 例えば、以下のようにすすめるのです
    - Domain層を直す
      - ドキュメントから必要な機能を確認する
      - エンティティと値オブジェクトがどんな風に定義されているとよいか、テストを作成する
      - そのテストに通るような実装になっているか確認する（そうでなければ今の実装を修正する）
    - Application層を直す
      - 同上
    - Infrastructure層を直す
      - 同上
    - Presentation層を直す
      - 同上
- 上記のようにしてアプリケーションを作成、修正していきたいのですが、お願いすることはできますか？
- わからないこと、迷うことがあれば必ずQA方式で私に聞いてください！判断します！！

1: Entity の JSON シリアライズメソッド
Hive Repository が以下のメソッドを呼んでいますが、実装されていますか？

確認内容：

goal.dart に static Goal fromJson() メソッドがあるか
goal.dart に Map<String, dynamic> toJson() メソッドがあるか
milestone.dart と task.dart にも同様にあるか
回答お願いします：
実装されていますか？されていない場合は実装が必要です。

すべて実装自体はされています。ただし、DomainのEntityに書くのが良いかは少し迷っています。
また、現在の実装はAggregation（集約）の概念があまりない気がします。
/gpt_app/の下のコードは書きかけ（正常に動作はしない）ですが、Aggregationの考え方は持っています。
どちらのほうが良いのでしょうか？

Q2: Application層の UseCase Provider
Presentation層から UseCase を利用するために、Riverpod Provider で UseCase をラップする必要があります。

確認内容：

lib/application/providers/use_case_providers.dart ファイルが存在するか
以下のようなProvider定義がされているか：
回答お願いします：
このファイルと Provider定義は実装されていますか？

はい、実装はしています。

Q3: ValueObject のマジックナンバー
以下のようなマジックナンバーが存在していますか？

確認内容：

ValueObject（GoalTitle, TaskTitle など）で硬くコーディングされた100が存在するか
修正内容：
回答お願いします：
修正すべき ValueObject がどのくらいあるか、把握していますか？

はい、以下のように変数に切り出しているものは存在します。
